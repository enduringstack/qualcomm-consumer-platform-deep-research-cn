...
所有 89 个编号来源均来自 Qualcomm/SEC、操作系统责任主体或竞品对自身产品负责的页面，并须被正文实际引用。自动门禁验证来源 ID 与 URL 唯一、正文引用有定义、来源被使用，以及产品矩阵的证据 ID 可解析；它不能验证某个来源是否在语义上支撑相邻句意，语义匹配仍由人工逐句抽审。链接 403、429 或超时只报告为网络限制；404/410 才是硬失败，避免把无法访问伪装成已核验。
...
`assemble_report.sh` 按固定章节顺序生成合并版。长度门禁在去除代码、行内代码和 URL 后按 Unicode Han 字符计数，阈值为 50,000；引用门禁要求不少于 60 个来源、来源 ID 与 URL 唯一、正文无未定义引用、来源无未使用、CSV 证据 ID 全部有定义；重复门禁对规范化后至少 160 字的段落做精确查重。
